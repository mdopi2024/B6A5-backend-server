...
import { bookingService, CreateBookingInput, UpdateBookingStatusInput } from "./booking.services";
...
const updateBookingStatus = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
  const payload: UpdateBookingStatusInput = req.body;
  const userId = req?.user?.id;
  ...
  const data = await bookingService.updateBookingStatus(id as string, payload, userId as string, userRole );
  sendResponse(res, status.OK, "Booking status updated successfully", data);
});
